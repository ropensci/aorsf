...
penguins_orsf$bill_length_mm <- 
 as.numeric(penguins_orsf$bill_length_mm)
penguins_orsf$flipper_length_mm <-
 as.numeric(penguins_orsf$flipper_length_mm)
...
The difference between oblique random forests and axis-based random forests is most clear in their decision surface. The decision surface of an oblique forest is smooth:

```{r}

fit_tree <- orsf(penguins_numeric,
                 n_tree = 1,
                 formula = species ~ bill_length_mm + flipper_length_mm,
                 control = orsf_control_classification())

fit_tree <- orsf_update(fit_penguins, 
                        control = orsf_control_classification(),
                        n_tree = 1, 
                        tree_seeds = 358815)
...
pred_tree <- predict(fit_tree, new_data = grid, pred_type = 'prob')
pred_forest <- predict(fit_penguins, new_data = grid, pred_type = 'prob')

colnames(pred_forest) <- levels(penguins_orsf$species)
colnames(pred_tree) <- levels(penguins_orsf$species)

grid_forest <- bind_cols(grid, pred_forest) %>%
...
grid_tree <- bind_cols(grid, pred_tree) %>%
 pivot_longer(cols = c(Adelie,
                       Chinstrap,
                       Gentoo)) %>%
 group_by(flipper_length_mm, bill_length_mm) %>%
...
ggplot(grid_forest, aes(bill_length_mm, flipper_length_mm)) +
 ...
 theme(panel.grid = element_blank(),
       ...
       legend.position = '') + 
 labs(title = "Decision surface of an oblique forest")

ggplot(grid_tree, aes(bill_length_mm, flipper_length_mm)) +
 geom_point(data = penguins_orsf,
            aes(color = species, shape = species),
            size = 3,
            alpha = 0.8) +
...
This is because oblique trees create decision boundaries that are neither parallel or perpendicular to the axes of corresponding predictors. 

By comparison, an axis based forest's decision surface is sharper:
...
pred_ranger <- predict(fit_ranger, data = grid)$predictions

colnames(pred_ranger) <- levels(penguins_orsf$species)
...
ggplot(grid_ranger, aes(bill_length_mm, flipper_length_mm)) +
 geom_point(data = penguins_orsf,
            aes(color = species, shape = species),
            size = 3,
            alpha = 0.8) +
 ...
 theme(panel.grid = element_blank(),
       panel.border = element_rect(fill = NA),
       legend.position = '') + 
 labs(title = "Decision surface of an axis-based forest")
...
We'll use the the `ames` housing data from the `modeldata` R package.
