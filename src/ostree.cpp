...
//
// // [[Rcpp::export]]
// arma::mat x_scale_cph_unwtd(arma::mat& x_mat){
//
//   // set aside memory for outputs
//   // first column holds the mean values
//   // second column holds the scale values
...
//     arma::vec x_i = x_mat.unsafe_col(i);
//
//     means(i) = arma::mean( x_i );
//
//     x_i -= means(i);
//
//     scales(i) = arma::sum(arma::abs(x_i));
//
//     if(scales(i) > 0)
//       scales(i) = x_mat.n_rows / scales(i);
//     else
//       scales(i) = 1.0; // rare case of constant covariate;
//
//     x_i *= scales(i);
...
//   int n_col = x.n_cols;
//
//   arma::mat x_transforms = x_scale_cph_unwtd(x);
//
//   double prob_sampled = 1.0/n_obs;
//
//   if(verbose == true){
