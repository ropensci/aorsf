...
- using in-bag predictions for the training data
  ...
  pd_inb <- orsf_pd_inb(fit, pd_spec = list(bili = 1:5))
...
- using out-of-bag predictions for the training data
  ...
  pd_oob <- orsf_pd_oob(fit, pd_spec = list(bili = 1:5))
...
pd_sex <- orsf_pd_oob(fit, pd_spec = list(sex = c("m", "f")))
...
pd_sex_tv <- orsf_pd_oob(fit, pd_spec = list(sex = c("m", "f")),
...
If you want to compute PD marginally for multiple variables, just list the variable values in `pd_spec` and specify `expand_grid = FALSE`. 

```{r}

pd_two_vars <-  
 orsf_pd_oob(fit,
             pd_spec = list(sex = c("m", "f"), bili = 1:5),
             expand_grid = FALSE)
...
I am going to turn off boundary checking in `orsf_ice_oob` by setting `boundary_checks = FALSE`, and this will allow me to generate ICE curves that go beyond the 90th percentile of `bili`.
  ...
  pd_spec <- list(bili = seq(1, 10, length.out = 25))
  
  ice_oob <- orsf_ice_oob(fit, pd_spec, boundary_checks = FALSE)
                       
  ```

For plots, it is helpful to scale the ICEs data. I subtract the initial value of predicted risk (i.e., when `bili = 1`) from each observation's conditional expectation values. So,
