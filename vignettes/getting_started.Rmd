...
The `orsf_interaction()` function indicated that `ascites` and `bili` have a strong interaction score (second highest), and the `orsf_vi()` function indicated that `bili` is the most important predictor. We can use the `orsf_pd()` function (pd stands for partial dependence) to explore how these variables influence predicted risk from ORSF.
...
ggplot(pd_data) +
 aes(x = bili, y = median, col = ascites) +
 geom_line() + 
