...
run_cph_test <- function(x, y, method){
 ...
 wts <- sample(seq(1:5), size = nrow(x), replace = TRUE)
 ...
 expect_equal(as.numeric(tt$coefficients), bcj$beta, tolerance = 1e-9)

 expect_equal(diag(tt$var),  bcj$var, tolerance = 1e-9)
...
test_that(
 ...
 code = {
  run_cph_test(x, y, method = 0)
  run_cph_test(x, y, method = 1)
 }
)
